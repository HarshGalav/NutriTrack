...
export class BarcodeService {
  ...
  async startContinuousDecoding(
    ...
    try {
      await this.reader.decodeFromVideoDevice(null, videoElement, (result, error) => {
        if (result) {
  ...
  private formatOpenFoodFactsData(product: Record<string, unknown>, barcode: string): ScannedProduct {
    const nutriments = (product.nutriments as Record<string, number>) || {}
    const servingSize = (product.serving_size as string) || (product.product_quantity as string) || '100'
